<template>
	<div>
	 ...
	 <div class="duanzi" v-for="itm in pinglun.comments">
      <div class="duanzi_diva">
        <p class="p1"><img width="50" :src="itm.headurl" alt=""></p>
        <p class="p2"><span>{{itm.nickname}}</span><span>{{itm.modifyTime | suma}}</span></p>
      </div>
...
export default {
	...
	methods:{
		shuju(){
        const url = '/plapi/Movie/HotLongComments.api?pageIndex='+this.page+'&movieId='+this.movieId;
        axios.get(url)
          .then(response=>{
